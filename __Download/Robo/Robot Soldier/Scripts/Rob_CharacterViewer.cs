...
public class Rob_CharacterViewer : MonoBehaviour {
	...
	void Update () {
        ...
        if (Input.GetAxis("Mouse ScrollWheel")!=0)
        {
            ...
            if (Input.GetAxis("Mouse ScrollWheel") < 0 && deltaPosition.y < 8) y = Input.GetAxis("Mouse ScrollWheel");
            if (Input.GetAxis("Mouse ScrollWheel") > 0 && deltaPosition.y > 0.1f) y = Input.GetAxis("Mouse ScrollWheel");
           deltaPosition = new Vector3(deltaPosition.x, deltaPosition.y - y * 4, deltaPosition.z + y * 8);
        }
	...
	void LateUpdate () {
	  ...
	  cameras.localPosition   =new Vector3(cameras.transform.localPosition .x ,
	  cameras.transform.localPosition .y+((targetForCamera.position.y + deltaPosition.y - cameras.localPosition .y) * Time.unscaledDeltaTime * 5),
	  	  cameras.transform.localPosition .z+((targetForCamera.position.z + deltaPosition.z - cameras.localPosition .z) * Time.unscaledDeltaTime * 5));
