    ...
    public class DepthCamera : MonoBehaviour
    {
        ...
        void ResetProj(Renderer renderer)
        {
            ...
            foreach (Material material in renderer.sharedMaterials)
            {
                if (material == null)
                    return;
                material.SetFloat("_ShadowOffset" + shadowIndex.ToString(), 0.0f);
                material.SetMatrix("_ShadowProj" + shadowIndex.ToString(), Matrix4x4.identity);
                material.SetTexture("_ShadowTex" + shadowIndex.ToString(), null);
            }
        ...
        void UpdateProj(Renderer renderer)
        {
            ...
            foreach (Material material in renderer.sharedMaterials)
            {
                if (material == null)
                    return;
                material.SetFloat("_ShadowOffset" + shadowIndex.ToString(), shadowOffset);
                material.SetMatrix("_ShadowProj" + shadowIndex.ToString(), lightProj);
                material.SetTexture("_ShadowTex" + shadowIndex.ToString(), cam.targetTexture);
            }
        }
